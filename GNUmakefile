...
obj/${IMAGE}: obj/$(KERNEL)
	...
	cp obj/$(KERNEL) obj/iso/boot
	cp -v limine.cfg limine/limine-bios.sys limine/limine-bios-cd.bin \
	    limine/limine-uefi-cd.bin obj/iso/boot/limine/
	mkdir -p obj/iso/EFI/BOOT
	cp -v limine/BOOTX64.EFI obj/iso/EFI/BOOT/
	cp -v limine/BOOTIA32.EFI obj/iso/EFI/BOOT/
